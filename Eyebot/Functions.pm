...
use HTTP::Request;
use LWP::UserAgent;
use Eyebot::Configure;
use strict;


our %conf = ( );
our $cfg = Eyebot::Configure->new(conffile => "Eyebot.conf");
%conf = $cfg->loadconf();
our $pattern;
our @list : shared = ( );
...
sub GetResponse(){
	...
	while($q->pending()){
		...
		if($url1 =~/^https:\/\//){
			...
			my $ua=LWP::UserAgent->new(agent => "Eyebot ".$conf{'version'}." http://www.Eyebot.com.br/");
...
sub CheckUpdate(){
	my $self = shift;
	my $h = Eyebot::Http->new();
	my $response = $h->GET("http://www.Eyebot.com.br/version.txt");
	chomp $response;
	if($response != $conf{'version'}){
		&write("self", "New version $response is avaliable");
		&write("self", "More details in http://www.Eyebot.com.br/\n\n");
	}
